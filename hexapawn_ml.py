...
#Remove , and \n
InitialMoveSet = [permutation.rstrip(",\n") for permutation in MoveSet[0:19]]
#Convert string to a tuple
InitialMoves = tuple(eval(",".join(InitialMoveSet)))


MoveSetConfigs = [permutation.rstrip(",\n") for permutation in MoveSet[19:38]]
ConfigMoveSet = tuple(eval(",".join(MoveSetConfigs)))
fh.close()

initialWinsLosses = {"Wins": 0, "Losses": 0}

file_path = "Scores.dat"
...
try:
    fh = open("Scores.dat", "rb+") #read and write in binary mode
except FileNotFoundError:
    f = open("Scores.dat", "wb") #write mode with binary encoding
    pickle.dump(initialWinsLosses, f) #save to file
    f.close()
    fh = open("Scores.dat", "rb+")

WinLoose = pickle.load(fh) #read from file
fh.close()

def update(WinOrLoose):
    file_path = "Scores.dat"
    temp_file_path = "temp.dat"
...
class Game:
    ...
    _initial_board = [BLACK, BLACK, BLACK, EMPTY, EMPTY, EMPTY, WHITE, WHITE, WHITE]
...
class GUI(Tk):
    def __init__(self, game):
        ...
        self.title("Hexapawn")
        self.resizable(0, 0)
        self.wpawn = PhotoImage(file="player.gif")
        self.bpawn = PhotoImage(file="computer1.gif")
        self.empty = PhotoImage(file="empty.gif")
        self.tk.call("wm", "iconphoto", self._w, self.wpawn)

        self.notice = StringVar()
        Label(
            self, textvariable=self.notice, width=45, height=2, font="Helvetica 12 bold"
        ).pack(pady=10)
        ...
        self.new_game_button = Button(self, text="NEW GAME", command=self.enable)
